...
<body>
    ...
    function GenerateRandKey($length = 10)
    {
        $keychar = [];
        $startingChar = 'A';
        for ($i = 0; $i < $length; $i++) {
    ...
    function GenerateIlluminaHash($msg)
    {
        $keystring = GenerateRandKey(strlen($msg));
    ...
    function CompareIlluminaHashes($leftHash, $rightHash)
    {
    ...
    function CreateUniqueDateTimeKey($add = 0)
    {
        ...
        $key =  $y . $M . $d . $h . $m;
        return GenerateIlluminaHash($key);
    }
    ...
    function VigenereCipher($msg, $keyword, $encryption = true)
    {
    ...
    function IlluminaCipherEncrypt($msg)
    {
        ...
        for ($j = 0; $j < 4; $j++) {
            ...
            $cipheredParts[$j] = VigenereCipher($cipheredParts[$j], $keywords[$j], true);
        }
    ...
    function IlluminaCipherDecrypt($raw)
    {
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $decipheredParts[$i] = VigenereCipher($decipheredParts[$i], $keywords[$i], false);
        }
        $decipheredChar = [];
        for ($j = 0; $j < $leng; $j++) {
            $sum = 0;
            for ($k = 0; $k < 4; $k++) {
                $sum += atoi(str_split($decipheredParts[$k])[$j]) - 65;
            }
            $sum += 32;
    ...
    $h1 = IlluminaCipherEncrypt("user=edrian&pass=edrian13.");
